...
void alm2cl_sp(sharp_alm_info * alm_info, float * alm, float * cl) {
	...
	int nl      = alm_info->lmax+1;
	float * buf = calloc(nthread*nl, sizeof(float));
	#pragma omp parallel
	{
		int id = omp_get_thread_num();
		if(id == 0) {
			for(int l = 0; l <= alm_info->lmax; l++) {
				ptrdiff_t i = alm_info->mvstart[0]*2 + l*2;
				buf[nl*id+l] = alm[i]*alm[i]/2;
			}
		}
		#pragma omp for schedule(dynamic)
		for(int m = 1; m < alm_info->nm; m++) {
			for(int l = m; l <= alm_info->lmax; l++) {
				ptrdiff_t i = alm_info->mvstart[m]*2 + l*2;
				buf[nl*id+l] += alm[i]*alm[i] + alm[i+1]*alm[i+1];
			}
		}
		#pragma omp barrier
		#pragma omp for
...
#if 0
void alm2cl_plain_sp(sharp_alm_info * alm_info, float * alm, float * cl) {
	// Straightforward implementation. The baseline we will try to improve on.
	// We don't support stride or packing.
	for(int l = 0; l <= alm_info->lmax; l++) {
		ptrdiff_t i = alm_info->mvstart[0]*2 + l*2;
		float c     = alm[i]*alm[i];
		for(int mi = 1; mi < alm_info->nm; mi++) {
			if(alm_info->mval[mi] > l) continue;
			i  = alm_info->mvstart[mi]*2 + l*2;
...
void alm2cl_mmajor_sp(sharp_alm_info * alm_info, float * alm, float * cl) {
	for(int l = 0; l <= alm_info->lmax; l++) {
		ptrdiff_t i = alm_info->mvstart[0]*2 + l*2;
		cl[l]       = alm[i]*alm[i];
	}
	for(int mi = 1; mi < alm_info->nm; mi++) {
		int m = alm_info->mval[mi];
		for(int l = m; l <= alm_info->lmax; l++) {
			ptrdiff_t i = alm_info->mvstart[mi]*2 + l*2;
...
void alm2cl_mmajor2_sp(sharp_alm_info * alm_info, float * alm, float * cl) {
	int nthread = omp_get_max_threads();
	int nl      = alm_info->lmax+1;
	float * buf = calloc(nthread*nl, sizeof(float));
	#pragma omp parallel
...
void alm2cl_avx_sp(sharp_alm_info * alm_info, float * alm, float * cl) {
	...
	for(int l1 = 0; l1 <= alm_info->lmax; l1+=8) {
		int l2 = min(l1+8, alm_info->lmax+1);
		int nm_block;
		if(l2 == l1+8) {
			// We have a whole block worth of ls. Handle all our ls up to nm_block
			nm_block = min(l1+1, alm_info->nm);
			...
			for(int m = 0; m < nm_block; m++) {
				...
				// Load our alms
				ptrdiff_t i = alm_info->mvstart[m]*2 + l1*2;
		...
		// General case
		for(int m = nm_block; m < alm_info->nm; m++) {
